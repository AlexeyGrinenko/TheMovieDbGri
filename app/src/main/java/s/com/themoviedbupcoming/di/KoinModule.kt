...
import s.com.themoviedbupcoming.data.repository.GenriesRepository
...
import s.com.themoviedbupcoming.presentation.search.SearchFragmentPresenter
import s.com.themoviedbupcoming.presentation.search.SearchScreenContract
import s.com.themoviedbupcoming.presentation.search.SearchScreenPresenter
import s.com.themoviedbupcoming.remote.ds.GenresDataSource
...
import s.com.themoviedbupcoming.ui.SearchActivity
...
//Router
    ...
    scope<ISearchScreenRouter>(KOIN_KEY_SCOPE_SEARCH_ACTIVITY) { (activity: SearchActivity) -> SearchScreenRouter(activity) }
...
// Presenter
    ...
    scope<SearchScreenContract.Presenter>(scopeId = KOIN_KEY_SCOPE_SEARCH_ACTIVITY) { (activity: BaseActivity) -> SearchScreenPresenter( get { parametersOf(activity) }) }

    scope<MoviesListContract.MoviesListPresenter>(scopeId = KOIN_KEY_SCOPE_MOVIES_LIST_FRAGMENT) { (activity: MainActivity) -> MoviesListPresenter(get(MoviesUseCase::class.java.name), get { parametersOf(activity) }) }
    ...
    scope<SearchFragmentContract.Presenter>(scopeId = KOIN_KEY_SCOPE_SEARCH_FRAGMENT) { (activity: BaseActivity) -> SearchFragmentPresenter(get(GenresUseCase::class.java.name), get { parametersOf(activity) }) }


//Use Case
    factory(name = MoviesUseCase::class.java.name) { MoviesUseCase(get(MoviesRepository::class.java.name), get()) }
    factory(name = MovieDetailUseCase::class.java.name) { MovieDetailUseCase(get(MovieDetailRepository::class.java.name), get()) }
    factory(name = GenresUseCase::class.java.name) { GenresUseCase(get(GenriesRepository::class.java.name), get()) }
...
//Repository
    ...
    factory<IGenriesRepository>(name = GenriesRepository::class.java.name) { GenriesRepository(get(GenresDataSource::class.java.name)) }
...
//Data source
    ...
    factory<IGenresDataSource>(name = GenresDataSource::class.java.name) { GenresDataSource(get(MoviesService::class.java.simpleName)) }
...
val KOIN_KEY_SCOPE_SEARCH_ACTIVITY = ScopeName.KOIN_KEY_SCOPE_SEARCH_ACTIVITY.name
...
val KOIN_KEY_SCOPE_SEARCH_FRAGMENT = ScopeName.KOIN_KEY_SCOPE_SEARCH_FRAGMENT.name
