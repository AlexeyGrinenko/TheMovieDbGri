...
class MoviesRepository(val moviesDataSource: IMoviesDataSource) : IMoviesRepository {
    ...
    override fun loadShows(isRefreshing: Boolean, searchModel: SearchModel): Single<List<MovieInList>> {
//        if (isRefreshing) {
//            currentPage = 0
//        }
        currentPage++
        return moviesDataSource.loadMovies(currentPage, searchModel).map { it.result }
    }
}